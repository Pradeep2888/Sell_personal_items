...
import { LOGIN, SEND_OTP, SIGNUP } from '../../services/operations/authApi';
...
function ForgetPassword() {
    ...
    const handleSendOTP = async (e) => {
        ...
        if (res?.status) {
            ...
            setTimeout(() => {
                navigate('/forget-password/otp-verification', { state: { email } });
                setLoading(false)
            }, 1000)
        } else {
            setLoading(false)
        ...
        <form autoComplete="off" className='w-full relative bg-white flex justify-center items-start rounded-b p-4 lg:p-12'>
            <div className='flex justify-center items-center relative overflow-hidden'>
                <div className='flex w-full flex-col gap-6'>
                    <h1 className='text-primary text-xl md:text-2xl lg:text-2xl font-bold text-center'>Forget Password
                        {/* With {accountType === 'DONOR' ? "Donor" : accountType === 'SELLER' ? "Seller" : "Buyer"} Account */}
                    </h1>
                    <div className='relative mt-10'>
                        <input ref={ref => inputrefs.current[0] = ref} required={true}
                            autoComplete="false"
                            className='py-4 w-full px-16 border border-[#D5E3EE] rounded focus:outline-none placeholder:text-[#374b5c] text-base font-medium placeholder:text-light'
                            type="text"
                            placeholder="Email"
                    ...
                    <button onClick={handleSendOTP} className='rounded w-full px-8 py-4 bg-[#537CD9] text-white font-bold'>{loading ? "Sending..." : "Send OTP"}</button>
                    {/* <button
                                        onClick={(e) => {
                                            e.preventDefault();
                                            setSendNext(false)
